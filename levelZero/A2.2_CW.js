console.log("A2.2_CW2")
console.log("---- ---- ----")
console.log("Excercise 1")

console.log("---- ---- ----")

console.log("1.1")
let rohitAge = 22
let rahulAge = 23

let isRohitOlder = rohitAge > rahulAge

console.log("Is Rohit older than Rahul? ", isRohitOlder)

console.log("---- ---- ----")

console.log("1.2")
let isRohitYounger = rohitAge < rahulAge

console.log("Is Rohit younger than Rahul? ", isRohitYounger)

console.log("---- ---- ----")

console.log("1.3")
let romaAge = 22

let isRohitEqualRoma = rohitAge === romaAge

console.log("Are Rohit and Roma of same age? ", isRohitEqualRoma)

console.log("---- ---- ----")

console.log("1.4")
let ninaAge = 23

let isNinaOlderOrSameRahul = ninaAge >= rahulAge

console.log("Is Nina older or equal to Rahul? ", isNinaOlderOrSameRahul)

console.log("---- ---- ----")

console.log("1.5")
let isNinaOlderOrSameRohit = ninaAge >= rohitAge

console.log("Is Nina older or equal to Rohit? ", isNinaOlderOrSameRohit)

console.log("---- ---- ----")

console.log("1.6")
let isRomaYoungerOrSameNina = romaAge <= ninaAge

console.log("Is Roma younger or equal to Nina? ")

console.log("---- ---- ----")
console.log("Excercise 2")
console.log("---- ---- ----")

console.log("2.1")
let product1Price = 499
let prodcut2Price = 699
let budget = 1199

let totalPrice = product1Price + prodcut2Price

let areBothUnderBudget = budget > totalPrice

console.log("Can I afford the two products? ", areBothUnderBudget)

console.log("---- ---- ----")

console.log("2.2")
let maths = 87
let english = 33
let science = 43
let passingAverage = 60
let average = (maths + english + science) / 3
let isPass = average > passingAverage

console.log("Did i pass? ", isPass)

console.log("---- ---- ----")

console.log("2.3")
let number = 50
let isNumberIsDivisibleBy3 = number % 3 === 0
let isNumberIsDivisibleBy7 = number % 7 === 0

console.log("Divisible by 3? ", isNumberIsDivisibleBy3)
console.log("Divisible by 7? ", isNumberIsDivisibleBy7)